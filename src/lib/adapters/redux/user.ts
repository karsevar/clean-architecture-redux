...
const updateHandler = (state: StateSlice, action: UpdateUserActionType): StateType['user'] => {
    return action.user;
};

export const userReducer = (state: StateSlice = initialState.user, action: UpdateUserActionType): StateSlice => {
    switch (action.type) {
        case UPDATE_USER:
